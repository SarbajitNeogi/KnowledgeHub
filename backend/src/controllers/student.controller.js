...
const verifyEmail = async (Email, Firstname, createdStudent_id) => {
    try {
        const emailsender = nodemailer.createTransport({
            ...
            requireTLS: true,
            auth: {
                user: "neogisarbajit080103@gmail.com", // Replace with your email
                pass: "glvpwwwocdtmydsa", // Replace with your email password or App Password
            }
        });
        // const mailOptions = {
    ...
    <div style="text-align: center;">
        ...
        <a href="http://localhost:8000/api/student/verify?id=${createdStudent_id}" 
           style="background-color: black; color: white; padding: 10px 20px; text-decoration: none; display: inline-block;">
           Verify Email
        </a>
    </div>`
};
...
const mailVerified = asyncHandler(async(req,res)=>{
    ...
    if (updatedInfo.modifiedCount === 0) {
        throw new ApiError(404, "Student already verified");
    }
...
const addStudentDetails = asyncHandler(async (req, res) => {
    try {
        ...
        console.log("Checking existing phone number...");
        ...
        const studentdetails = await studentdocs.create({
            Phone, Address, Highesteducation, SecondarySchool, HigherSchool, SecondaryMarks, HigherMarks,
            Aadhaar: Aadhaar.url, Secondary: Secondary.url, Higher: Higher.url
        });
...
const forgetPassword=asyncHandler(async(req,res)=>{
   ...
   <p>${resetToken}</p>
   <p>Thank you for being a valued member of the KnowledgeHub community. If you have any questions or need further assistance, please do not hesitate to contact our support team.</p>
   <p>Best regards,</p>
   <p>The KnowledgeHub Team</p>`
